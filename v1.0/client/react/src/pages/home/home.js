...
import { getPokemons, getPokemonsEnemies, requestSwapPokemon, updateLevelsAndPlays } from "../../utils/requests"
    ...
    firstTeam.forEach((e)=>{
        if(e.actualHP > 0){
            ...
            secondTeam[pokemonEnemy].actualHP = (
                secondTeam[pokemonEnemy].actualHP - e.power < 0 ? 0 : secondTeam[pokemonEnemy].actualHP - e.power
            )
...
const HomePage = ()=>{
    ...
    const handleAttack = async ()=>{
        const meanSpeedUser = pokemonsUser.battlefield.reduce((total, value)=>total+value.velocity,0) / 3
        const meanSpeedEnemy = pokemonsEnemy.reduce((total, value)=>total+value.velocity,0) / 3
        let firstTeam = []
        let secondTeam = []
        let liveEnemies = []
        ...
        if(liveEnemies.length === 0){
            if(userTeamWon){//win
                setDataModal({show:true, backgroundColor:"green", text:"You won the battle!!!"})
                //new enemies
                const refreshPokemons = await requestFinishedPlay(userID)

                if(refreshPokemons){
                    setDataModal({show:true, backgroundColor:"green", text:refreshPokemons})
                }
                const newPokemons = await getPokemonsUser(userID)
                setPokemonsUser(newPokemons)
            } else {
                setDataModal({show:true, backgroundColor:"red", text:"You lost the battle :("})
            ...
            if(newEnemies){
                ...
                setDataModal({show:true, backgroundColor:"green", text:"Error getting enemies pokemons"})
            }
    ...
    const swapPokemon = async (pokemonBattle)=>{
        ...
        const res = await requestSwapPokemon({inventory:stateSwapPokemon[0], battle:pokemonBattle, userID})
        if(!res.err){
            if(res.data){
                const localID = []
            ...
            setDataModal({show:true, backgroundColor:"red", text:"Error swapping in server"})
        }
    }
    ...
    useEffect(()=>{
        (async()=>{
            ...
            if(newPokemons){
                ...
                setDataModal({show:true, backgroundColor:"green", text:"Error getting pokemons"})
            }
        })()
