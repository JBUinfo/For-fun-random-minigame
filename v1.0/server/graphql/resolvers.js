...
const generateRandomValues = ()=>{
  ...
  pokemon.evolution_level = calcRandomNumber(pokemon.level+15, pokemon.level+45)
...
export const getResolvers = (database)=>{
  ...
  const insertPokemonToNewUser = async(user_id)=>{
    const pokemon = await new Promise((resolve, reject)=>{
      ...
      database.run(querys.insertToInventory,[
        random.pokemon_id,
        user_id,
        true,
        random.hp,
  ...
  const resolvers = {
    ...
    insertUser: async ({params})=>{
      ...
      if(user.error != 1){
        user = await new Promise((resolve, reject)=>{
          database.get(querys.selectUser, [params.nick,params.pass], (err,data)=>{
            if(err)resolve({data:null, error:1})
    ...
    getAllPokemons: async()=>{
      const pokemons = await new Promise((resolve, reject)=>{
    ...
    getAllPokemonsFromInventory: async({user_id})=>{
      const pokemons = await new Promise((resolve, reject)=>{
        database.all(querys.selectPokemonsFromUser,[user_id], (err,data)=>{
          if(err) resolve({data:null, error:1})
          resolve({data:data, error:err})
        })
      })
    ...
    getRandomEnemies: async()=>{
      const randoms = [
        ...
        generateRandomValues(),
        generateRandomValues()].sort((a,b)=>a.pokemon_id - b.pokemon_id)
      const pokemons = await new Promise((resolve, reject)=>{
        database.all(querys.selectEnemiesPokemons,[
          randoms[0].pokemon_id,
          randoms[1].pokemon_id,
          randoms[2].pokemon_id], (err,data)=>{
          if(err) resolve({data:null, error:1})
    ...
    updatePlays: async({params})=>{
      const pokemonsLevelUp = await new Promise((resolve, reject)=>{
        database.run(querys.updateLevel,[
          params.user_id], (err,data)=>{
          if(err) resolve({data:null, error:1})
          resolve({data:data, error:err})
        })
      })
      if(pokemonsLevelUp.err == null){
        const evolutions = await new Promise((resolve, reject)=>{
          let thereAreEvolutions = false
          database.all(querys.selectSelectedPokemonsFromUser,[
            params.user_id], (err,data)=>{
              if(err) resolve({data:null, error:1})
              ...
              resolve({data:thereAreEvolutions, error:err})
          })
        })
        if(evolutions.error == null){
          const updatePlays = await new Promise((resolve, reject)=>{
            database.run(querys.updatePlays,[
              params.user_id], async (err,data)=>{
              if(err) resolve({data:null, error:1})
              database.get(querys.selectUserFromID, [params.user_id],
                async (err, data)=>{
                  if(err) resolve({data:null, error:1})
                  if(data.plays%10 == 0){
    ...
    swapPokemons:async({params})=>{
      const inventoryToBattle = await new Promise((resolve, reject)=>{
        database.run(querys.swapInventoryToBattle,[params.user_id, params.inventory], (err,data)=>{
          if(err) resolve({data:null, error:1})
          resolve({data:true, error:null})
        })
      })
      if(inventoryToBattle.data){
        const battleToInventory = await new Promise((resolve, reject)=>{
          database.run(querys.swapBattleToInventory,[params.user_id, params.battle], (err,data)=>{
            if(err) resolve({data:null, error:1})
            resolve({data:true, error:null})
          })
        })
